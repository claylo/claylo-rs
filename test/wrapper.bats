...
@test "wrapper: new with --data passes through to copier" {
    ...
    assert_success
    assert_line --partial "--data edition=2021"
    assert_line --partial "--data msrv=1.80.0"
}
...
@test "wrapper: update builds correct copier update command" {
    ...
    assert_success
    assert_line --partial "Running: copier update --trust"
    # update should NOT include --defaults
    refute_line --partial "--defaults"
}
...
@test "wrapper: +otel enables has_opentelemetry" {
    ...
    assert_success
    assert_line --partial "--data has_opentelemetry=true"
}

@test "wrapper: -site disables has_site" {
    PATH="${MOCK_DIR}:${PATH}" run "$WRAPPER" new ./foo -site
    assert_success
    assert_line --partial "--data has_site=false"
}
...
@test "wrapper: compound feature string +otel-site+bench" {
    ...
    assert_line --partial "--data has_opentelemetry=true"
    assert_line --partial "--data has_site=false"
    assert_line --partial "--data has_benchmarks=true"
}
...
@test "wrapper: all feature aliases resolve correctly" {
    ...
    assert_line --partial "--data has_jsonl_logging=true"
    assert_line --partial "--data has_claude_skills=true"
    assert_line --partial "--data has_env_files=true"
    assert_line --partial "--data has_issue_templates=true"
    assert_line --partial "--data has_md_strict=true"
    assert_line --partial "--data has_skill_using_git=true"
}
...
@test "wrapper: features work with update command" {
    ...
    assert_success
    assert_line --partial "--data has_opentelemetry=true"
    assert_line --partial "--data has_community_files=false"
}
