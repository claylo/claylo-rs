# `scripts/add-crate` Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create a bash script that adds new crates to an existing Rust workspace, with claylo-rs convention detection.

**Architecture:** A single bash script at `template/scripts/add-crate` that discovers workspace layout, prompts for crate details (or accepts flags), generates `Cargo.toml` + `src/` files, and validates the result with `cargo check`. Convention detection via `.repo.yml` presence. Ships in every generated project.

**Tech Stack:** Bash, TOML generation (heredocs), cargo CLI for validation.

**Design doc:** `docs/plans/2026-02-27-add-crate-design.md`

---

### Task 1: Core Script Skeleton

**Files:**
- Create: `template/scripts/add-crate`
...
Run: `bash -n template/scripts/add-crate`
Expected: No output (syntax OK)

**Step 3: Make executable**

Run: `chmod +x template/scripts/add-crate`

**Step 4: Commit**

```bash
git add template/scripts/add-crate
git commit -m "feat(add-crate): script skeleton with argument parsing"
```

---

### Task 2: Workspace Discovery & Convention Detection

**Files:**
- Modify: `template/scripts/add-crate`

**Step 1: Add workspace detection functions**
...
if [[ "$HAS_WORKSPACE_PACKAGE" == true ]]; then
    EDITION=$(grep '^edition' "$WORKSPACE_ROOT/Cargo.toml" | head -1 | sed 's/.*= *"\?\([^"]*\)"\?/\1/')
    MSRV=$(grep '^rust-version' "$WORKSPACE_ROOT/Cargo.toml" | head -1 | sed 's/.*= *"\?\([^"]*\)"\?/\1/')
fi
...
**Step 2: Test detection in a generated project**
...
```bash
git add template/scripts/add-crate
git commit -m "feat(add-crate): workspace discovery and convention detection"
```
...
Append after workspace discovery:
...
```bash
git add template/scripts/add-crate
git commit -m "feat(add-crate): crate name validation"
```
...
**Step 1: Add interactive prompt functions**
...
```bash
git add template/scripts/add-crate
git commit -m "feat(add-crate): interactive prompts with validation"
```
...
**Step 1: Add Cargo.toml generation**

Append after prompts:
...
```bash
git add template/scripts/add-crate
git commit -m "feat(add-crate): Cargo.toml and source file generation"
```

---

### Task 6: Workspace Integration & Output

**Files:**
- Modify: `template/scripts/add-crate`

**Step 1: Add workspace member check and cargo validation**

Append after file generation:
...
```bash
git add template/scripts/add-crate
git commit -m "feat(add-crate): workspace integration, validation, and output"
```

---

### Task 7: Add Justfile Recipe

**Files:**
- Modify: `template/.justfile.jinja`

**Step 1: Add the `add-crate` recipe**

Add before the `release-check` recipe (around line 230), after the site recipes:

```just
# Add a new crate to the workspace
add-crate *ARGS:
    scripts/add-crate {% raw %}{{ARGS}}{% endraw %}
```

**Step 2: Verify the justfile template is still valid Jinja**

Run: `just lint` (checks copier.yaml formatting — justfile changes don't need this, but verify no accidental copier.yaml edits)

**Step 3: Commit**

```bash
git add template/.justfile.jinja
git commit -m "feat(add-crate): add just add-crate recipe"
```

---

### Task 8: Template-Level Test

**Files:**
- Modify: `test/conditional_files.bats`

**Step 1: Add test that the script is always present**

Add to `conditional_files.bats` (alongside the existing always-present file tests):

```bash
@test "add-crate script is always present" {
    local output_dir
    ...
    local project_name
    project_name=$(get_project_name "$output_dir")
    [[ -x "$output_dir/$project_name/scripts/add-crate" ]]
}

@test "add-crate script present in all presets" {
...
Run: `./test/bats/bin/bats -F "$(pwd)/test/formatters/agents.bash" test/conditional_files.bats`
...
git commit -m "test(add-crate): verify script ships in all presets"
```
...
**Step 1: Add a functional test that runs the script**

```bash
@test "add-crate creates a library crate" {
    local output_dir
    output_dir=$(generate_project_with_data "cond-add-crate-lib" "standard.yml")
    local project_name
    project_name=$(get_project_name "$output_dir")
    ...
    # Verify it compiles
    cargo check -p my-utils
}

@test "add-crate creates a binary crate" {
    local output_dir
    output_dir=$(generate_project_with_data "cond-add-crate-bin" "standard.yml")
    local project_name
    project_name=$(get_project_name "$output_dir")
    local project_dir="$output_dir/$project_name"
    ...
    # Verify it compiles
    cargo check -p my-daemon
}

@test "add-crate rejects duplicate crate names" {
    local output_dir
    output_dir=$(generate_project_with_data "cond-add-crate-dup" "standard.yml")
    local project_name
    project_name=$(get_project_name "$output_dir")
    local project_dir="$output_dir/$project_name"

    cd "$project_dir"
    # The project itself has a crate with the project_name
    run ./scripts/add-crate lib "$project_name" -d "Duplicate"
    [[ "$status" -ne 0 ]]
}
```
...
Run: `./test/bats/bin/bats -F "$(pwd)/test/formatters/agents.bash" test/conditional_files.bats`
Expected: All tests pass. The functional tests may be slow (cargo check). If so, consider whether they should move to `presets.bats` instead.

**Step 3: Commit**

```bash
git add test/conditional_files.bats
git commit -m "test(add-crate): functional tests for lib, bin, and duplicate detection"
```
...
**Files:**
- Modify: `template/scripts/add-crate` (if any issues found in testing)
- No docs changes needed — the script has `--help` built in

**Step 1: Run the full test suite**

Run: `just test-fast`
Expected: All conditional file tests pass.

**Step 2: Run preset tests to verify no regressions**

Run: `just test-presets`
Expected: All preset tests pass (the script is always present but doesn't affect compilation).

**Step 3: Final commit if any polish needed**

```bash
git add -A
git commit -m "fix(add-crate): polish from test feedback"
```
